"""delivery.datadog_events: the summary back on the timeline beside its monitors."""
import json
...
def dd(monkeypatch):
    ...
    mod = load_function_module("summarizer", "delivery.datadog_events")
    yield mod, mock_table, mock_secrets


def _post(mod, incident=INCIDENT, recovered=False, event_id=555):
    with patch("requests.post", return_value=_mock_dd_response(event_id)) as mock_post:
        result = mod.deliver(dict(incident), recovered)
    return result, mock_post


def _posted_event(mod, incident=INCIDENT, recovered=False) -> dict:
    _, mock_post = _post(mod, incident, recovered)
...
class TestDeliver:
    def test_posts_event_and_returns_event_id(self, dd):
        mod, _, _ = dd
        result, _ = _post(mod, event_id=9001)
        assert result == "9001"

    def test_writes_event_id_to_dynamodb_and_the_incident(self, dd):
        mod, mock_table, _ = dd
        ...
        assert incident["datadog_event_id"] == "9001"
    ...
    def test_http_error_propagates_and_nothing_is_written(self, dd):
        ...
        with patch("requests.post", return_value=mock_response):
            with pytest.raises(requests.HTTPError):
                mod.deliver(dict(INCIDENT), False)
...
class TestPayload:
    ...
    def test_aggregation_key_is_the_incident(self, dd):
        mod, _, _ = dd
        assert _posted_event(mod)["aggregation_key"] == "incident:inc-123"

    def test_text_is_markdown_fenced(self, dd):
        mod, _, _ = dd
        text = _posted_event(mod)["text"]
    ...
    def test_text_omits_links_that_do_not_exist_yet(self, dd, monkeypatch):
        ...
        text = _posted_event(mod, INCIDENT_NO_LINKS)["text"]
    ...
    def test_text_falls_back_when_no_summary(self, dd):
        mod, _, _ = dd
        text = _posted_event(mod, INCIDENT_NO_SUMMARY)["text"]
        assert "No LLM summary available" in text
        assert "high-error-rate (cloudwatch)" in text

    def test_text_falls_back_when_summary_is_malformed(self, dd):
        mod, _, _ = dd
        text = _posted_event(mod, {**INCIDENT, "llm_summary": "not json"})["text"]
        assert "No LLM summary available" in text
    ...
    def test_text_is_truncated_to_the_datadog_limit_keeping_links(self, dd):
        ...
        text = _posted_event(mod, huge)["text"]
...
class TestSeverityMapping:
    ...
    def test_severity_maps_to_alert_type_and_priority(self, dd, severity, alert_type, priority):
        mod, _, _ = dd
        event = _posted_event(mod, {**INCIDENT, "severity": severity})
...
class TestRecovery:
    def test_recovery_event_is_a_success_on_the_same_aggregation_key(self, dd):
        mod, _, _ = dd
        event = _posted_event(mod, RESOLVED_INCIDENT, recovered=True)
    ...
    def test_open_event_tags_status_open(self, dd):
        mod, _, _ = dd
        assert "status:open" in _posted_event(mod)["tags"]

    def test_recovery_event_id_written_back(self, dd):
        mod, mock_table, _ = dd
        result, _ = _post(mod, RESOLVED_INCIDENT, recovered=True, event_id=777)
        assert result == "777"
    ...
    def test_recovery_without_summary_still_posts(self, dd):
        mod, _, _ = dd
        event = _posted_event(mod, {k: v for k, v in RESOLVED_INCIDENT.items() if k != "recovery_summary"}, recovered=True)
